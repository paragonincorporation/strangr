...
test("public landing reaches the auth shell", async ({ page }) => {
  await page.goto("/");
  await expect(
    page.getByRole("heading", { name: "Meet on Paramingle." }),
  ).toBeVisible();
  await page.getByRole("link", { name: "Create your account ↗" }).click();
  await expect(
    page.getByRole("heading", { name: "Come meet someone new." }),
  ).toBeVisible();
});
...
test("account shell exposes core destinations", async ({ page }) => {
  await page.goto("/app");
  await expect(
    page.getByRole("heading", { name: "Meet someone new." }),
  ).toBeVisible();
  await expect(
...
test("report actions remain distinct in the call room", async ({ page }) => {
  ...
  await expect(
    page.getByRole("button", { name: "Submit and leave" }),
  ).toBeVisible();
});
