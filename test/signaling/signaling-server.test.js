...
test('real sockets pair, relay chat, and cleanly next without stale state', async () => {
  ...
  const queue = new MatchmakingQueue({ ttlMs: 5_000, recentPairs, random: () => 0 })
  ...
  await Promise.all([waitForType(first, 'session.ready'), waitForType(second, 'session.ready')])
  ...
  send(first, 'chat.message', { pairId: firstMatch.pairId, text: '<b>still plain text</b>' })
...
test('a paired session resumes on a new socket inside the grace window', async () => {
  ...
  const relayedChat = waitForType(second, 'chat.message')
  send(replacement, 'chat.message', { pairId: match.pairId, text: 'back online' })
