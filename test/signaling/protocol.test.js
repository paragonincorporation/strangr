...
test("protocol rejects malformed or unknown envelopes", () => {
  ...
  assert.equal(
    parseEnvelope(
      Buffer.from(
        JSON.stringify({ type: "match.join", payload: { mode: "video" } }),
      ),
    ).ok,
    true,
  );
...
test("chat cleaner strips control bytes and caps length without interpreting HTML", () => {
  const value = cleanChatText(
    `\u0000<script>alert(1)</script>${"x".repeat(600)}`,
  );
