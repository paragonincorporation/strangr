...
chatInput.addEventListener('input', () => {
  ...
  typingTimer = setTimeout(() => socket?.send('chat.typing', { typing: false }), 800)
})
...
document.addEventListener('click', (event) => {
  ...
  if (action === 'toggle-chat') {
    ...
    document.querySelector('[data-chat-count]').textContent = open ? 'OPEN' : 'CLOSED'
  }
...
function createSocket() {
  ...
  socket.addEventListener('socket.status', (event) => {
    ...
    if (status === 'closed' && ['connected', 'reconnecting'].includes(machine.state)) {
      if (machine.state === 'connected') machine.transition('SOCKET_LOST')
      showReconnectNotice('YOUR SIGNAL DIPPED.', 'We’re retrying now. Wait here or skip as soon as the channel returns.')
      ui.status.set('Reconnecting', 'waiting')
    }
    if (status === 'error' && machine.state === 'matching') ui.matching('socket', store.mode)
  })
  ...
  socket.addEventListener('chat.typing', (event) => ui.chat.setTyping(Boolean(event.detail.typing)))
  socket.addEventListener('media.state', (event) => {
    document.querySelector('[data-peer-media]').textContent = event.detail.video ? 'CAMERA ON' : 'CAMERA OFF'
  })
  ...
  socket.addEventListener('error', (event) => {
    if (event.detail?.code !== 'stale_pair') ui.toast(event.detail?.message || 'Something got crossed. Retrying.')
  })
}
...
async function onMatch(data) {
  ...
  try {
    ...
    peer = new PeerConnection({
      ...
      sendSignal: (signal) => socket?.send('rtc.signal', { pairId: store.pairId, signal }),
    })
    peer.addEventListener('remote.stream', (event) => ui.videos.setRemote(event.detail.stream))
    peer.addEventListener('connection.state', (event) => onRtcState(event.detail.state))
...
function onPeerStatus(data) {
  if (data.status === 'reconnecting') {
    if (machine.state === 'connected') machine.transition('SOCKET_LOST')
    showReconnectNotice('STRANGR DROPPED.', 'Their signal is shaky. Wait here or move on now.', data.graceMs)
  ...
  if (data.status === 'resumed') {
    ...
    ui.hideNotice()
    ui.status.set('Connected', 'connected')
    ui.chat.addStatus('Strangr is back.')
  }
  if (data.status === 'left') {
    clearInterval(reconnectTimer)
    peer?.close()
    peer = null
    store.clearPair()
    if (machine.state === 'connected' || machine.state === 'reconnecting') machine.transition('PEER_LEFT')
    ui.status.set('Strangr left', 'danger')
    ui.showNotice({
      title: 'STRANGR LEFT.',
      copy: 'That one’s over. Find someone new whenever you’re ready.',
      canWait: false,
    })
    document.querySelector('[data-reconnect-countdown]').textContent = 'No chat history was saved.'
  }
}

function showReconnectNotice(title, copy, graceMs = ENV.reconnectGraceMs) {
  clearInterval(reconnectTimer)
  const deadline = Date.now() + Math.min(graceMs || ENV.reconnectGraceMs, 60_000)
  const tick = () => {
...
function nextStranger() {
  ...
  socket.send(hadPair ? 'match.next' : 'match.join', hadPair ? {} : { mode: store.mode })
}
