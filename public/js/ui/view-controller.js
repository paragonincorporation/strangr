...
export class ViewController {
  constructor(root = document) {
    this.root = root;
    this.views = new Map(
      [...root.querySelectorAll("[data-view]")].map((view) => [
        view.dataset.view,
        view,
      ]),
    );
  ...
  matching(stage, mode = "video") {
    ...
    this.root.querySelector("[data-match-mode]").textContent =
      mode === "video" ? "Video ready" : "Text-only mode";
  }
  ...
  enterRoom(mode) {
    ...
    this.chat.reset();
    const chatOpen = this.chat.toggle(
      mode === "text" || !window.matchMedia("(max-width: 980px)").matches,
    );
    this.root.querySelector("[data-chat-count]").textContent = chatOpen
      ? "OPEN"
      : "CLOSED";
    this.videos.setTextMode(mode === "text");
    this.root.querySelector("[data-peer-media]").textContent =
      mode === "text" ? "TEXT ONLY" : "CAMERA ON";
    this.hideNotice();
    this.status.set(
      mode === "text" ? "Text connected" : "Linking video",
      mode === "text" ? "connected" : "waiting",
    );
  }
  ...
  showNotice({ title, copy, countdown = "", canWait = true }) {
    ...
    this.root.querySelector("[data-reconnect-countdown]").textContent =
      countdown;
  ...
  updateMedia({ audio, video }) {
    this.root.querySelector("[data-audio-label]").textContent = audio
      ? "ON"
      : "OFF";
    this.root.querySelector("[data-video-label]").textContent = video
      ? "ON"
      : "OFF";
    this.root.querySelector("[data-local-media]").textContent = video
      ? "LIVE"
      : "CAM OFF";
    this.root
      .querySelector('[data-action="toggle-audio"]')
      .setAttribute("aria-pressed", String(!audio));
    this.root
      .querySelector('[data-action="toggle-video"]')
      .setAttribute("aria-pressed", String(!video));
