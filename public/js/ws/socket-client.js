...
const SESSION_KEY = 'strangr.socket-session'
...
export class SocketClient extends EventTarget {
  ...
  connect() {
    ...
    socket.addEventListener('message', (event) => {
      ...
      if (!envelope) return
      if (envelope.type === 'session.ready') this.#saveCredentials(envelope.payload)
      if (envelope.type === 'session.resume_failed') sessionStorage.removeItem(SESSION_KEY)
      this.dispatchEvent(new CustomEvent('message', { detail: envelope }))
      this.dispatchEvent(new CustomEvent(envelope.type, { detail: envelope.payload }))
    })
  ...
  #status(status) {
    this.dispatchEvent(new CustomEvent('socket.status', { detail: { status } }))
  }
}
