export class MediaManager extends EventTarget {
  ...
  state() {
    return {
      audio: Boolean(this.stream?.getAudioTracks().some((track) => track.enabled && track.readyState === 'live')),
      video: Boolean(this.stream?.getVideoTracks().some((track) => track.enabled && track.readyState === 'live')),
    }
  }
