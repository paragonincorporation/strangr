export class PeerConnection extends EventTarget {
  constructor({ iceServers, localStream, sendSignal }) {
    ...
    this.pc.addEventListener('track', (event) => {
      ...
      this.dispatchEvent(new CustomEvent('remote.stream', { detail: { stream: this.remoteStream } }))
    })
    this.pc.addEventListener('icecandidate', (event) => {
      if (event.candidate) this.sendSignal({ candidate: event.candidate })
    })
    this.pc.addEventListener('connectionstatechange', () => {
      this.dispatchEvent(new CustomEvent('connection.state', { detail: { state: this.pc.connectionState } }))
    })
  }
