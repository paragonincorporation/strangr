...
import type { ServerConfig } from '@strangr/config'
...
export function createSupabaseTokenVerifier(config: ServerConfig): TokenVerifier {
  const keys = createRemoteJWKSet(new URL(config.SUPABASE_JWKS_URL), { cooldownDuration: 30_000 })
  return {
    async verify(token) {
...
export function identityFromClaims(payload: JWTPayload): VerifiedIdentity {
  ...
  return {
    ...
    provider,
    authSessionId:
      typeof payload.session_id === 'string' ? payload.session_id : (payload.jti ?? payload.sub),
  }
}

export type Capability = 'inspect_self' | 'profile_setup' | 'contact'
export function capabilityError(state: AccountState, verified: boolean, capability: Capability) {
  if (state === 'banned' || state === 'deleted') return 'account_banned' as const
  if (state === 'suspended') return 'account_suspended' as const
  if (state === 'limited' && capability === 'contact') return 'account_limited' as const
  ...
  if ((state === 'pending_verification' || state === 'onboarding') && capability === 'contact')
