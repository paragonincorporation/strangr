...
import { avatarUploads, profiles } from "@paramingle/database";
...
export class SupabaseStorage implements ObjectStorage {
  ...
  async get(key: string) {
    const r = await fetch(
      `${this.baseUrl}/storage/v1/object/authenticated/${this.bucket}/${key}`,
      {
...
export class AvatarService {
  ...
  async upload(
    ...
    if (
      ...
      body.byteLength !== upload.declaredByteSize
    )
  ...
  async signedAvatar(userId: string) {
    ...
    if (!p?.avatarObjectKey)
      throw new DomainError("not_found", "Avatar not found", 404);
  ...
  async cleanup(now = new Date()) {
    const rows = await this.db
      ...
      .where(
        and(
          eq(avatarUploads.state, "pending"),
          lt(avatarUploads.expiresAt, now),
        ),
      );
