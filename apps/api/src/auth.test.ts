...
describe('authentication and age policy', () => {
  ...
  test('gates contact by verification, onboarding, and sanctions', () => {
    ...
    expect(capabilityError('active', false, 'contact')).toBe('email_unverified')
    expect(capabilityError('onboarding', true, 'contact')).toBe('onboarding_required')
    expect(capabilityError('suspended', true, 'inspect_self')).toBe('account_suspended')
  })
  ...
  test('derives cohort at UTC birthday boundaries', () => {
    ...
    expect(() => deriveAge('2011-07-13', new Date('2026-07-13T00:00:00Z'))).not.toThrow()
  })
})
