...
export function createReportHandler({ sessions, reportLimiter, logger = logReport, now = Date.now }) {
  return (request, response) => {
    const { reportToken, reason } = request.body || {}
    const session = [...sessions.sessions.values()].find((candidate) => candidate.reportToken === reportToken)
    ...
    if (!limit.allowed) {
      ...
      return response.status(429).json({ error: 'rate_limited', retryAfterMs: limit.retryAfterMs })
    }
