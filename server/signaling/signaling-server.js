...
export class SignalingServer {
  ...
  #onConnection(socket) {
    ...
    socket.on("message", (raw) => {
      ...
      if (!parsed.ok) {
        send(socket, "error", {
          code: parsed.code,
          message: "That message could not be processed.",
        });
        return;
      }
  ...
  #handle(socket, envelope) {
    ...
    if (type === "chat.typing")
      this.#relay(session, "chat.typing", { typing: Boolean(payload.typing) });
    if (type === "media.state")
      this.#relay(session, "media.state", {
  ...
  #join(session, mode, requestId) {
    if (!["video", "text"].includes(mode)) {
      send(
        session.socket,
        "error",
        { code: "invalid_mode", message: "Choose video or text chat." },
        requestId,
      );
      return;
    }
    ...
    if (!limit.allowed) {
      send(
        session.socket,
        "rate_limited",
        { scope: "matching", retryAfterMs: limit.retryAfterMs },
        requestId,
      );
      return;
    }
  ...
  #pair(first, second, mode) {
    ...
    send(first.socket, "match.found", {
      pairId,
      mode,
      role: "initiator",
      reportToken: firstReportToken,
    });
    send(second.socket, "match.found", {
      pairId,
      mode,
      role: "responder",
      reportToken: secondReportToken,
    });
  }
  ...
  #relayRtc(session, payload, requestId) {
    if (
      ...
      send(
        session.socket,
        "error",
        { code: "stale_pair", message: "That connection is no longer active." },
        requestId,
      );
      return;
    }
    this.#relay(session, "rtc.signal", {
  ...
  #relayChat(session, payload, requestId) {
    ...
    if (!limit.allowed) {
      send(
        session.socket,
        "rate_limited",
        { scope: "chat", retryAfterMs: limit.retryAfterMs },
        requestId,
      );
      return;
    }
  ...
  #peerFor(session) {
    ...
    return this.sessions.get(
      pair.firstId === session.id ? pair.secondId : pair.firstId,
    );
  }
