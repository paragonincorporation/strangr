...
export const reportCreateSchema = z
  ...
  .refine(
    (value) =>
      [value.encounterId, value.callId, value.messageId].filter(Boolean)
        .length === 1,
    "Exactly one report context is required",
  );
...
export const sanctionCreateSchema = z
  ...
  .refine(
    (x) =>
      ...
      x.type === "verification_challenge",
    "A temporary sanction needs an end time",
  )
  .refine(
    (x) => !x.permanent || x.evidenceReferences.length > 0,
    "Permanent sanctions require evidence",
  );
...
export const matchingPreferencesResponseSchema =
  matchingPreferencesSchema.extend({
...
export function parseClientRealtimeMessage(input: string) {
  const bytes = utf8ByteLength(input);
  if (bytes > MAX_REALTIME_MESSAGE_BYTES)
    return { success: false as const, error: "payload_too_large" as const };
  try {
