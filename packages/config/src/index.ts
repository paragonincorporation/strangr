...
export function parseServerConfig(
  ...
  if (result.data.NODE_ENV === "production") {
    ...
    if (insecureOrigins.length)
      throw new Error(
        "Invalid server configuration: production origins must use https",
      );
    ...
    if (insecureSupabaseUrls.length)
      throw new Error(
        "Invalid server configuration: production Supabase URLs must use https",
      );
  }
  return result.data;
}
...
export const clientPublicConfigSchema = z
  ...
  .superRefine((value, context) => {
    if (value.VITE_DEPLOYMENT_ENVIRONMENT !== "production") return;
    for (const key of ["VITE_API_URL", "VITE_SUPABASE_URL"] as const)
...
export function parseClientPublicConfig(
