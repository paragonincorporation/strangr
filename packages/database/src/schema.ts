...
export const profiles = pgTable(
  ...
  (table) => [
    uniqueIndex("profiles_normalized_username_uidx").on(
...
export const entitlementGrants = pgTable(
  ...
  (table) => [
    uniqueIndex("entitlement_grants_source_uidx").on(
      table.userId,
...
export const userSessions = pgTable(
  ...
  (table) => [
    uniqueIndex("user_sessions_user_auth_session_uidx").on(
      table.userId,
      table.authSessionId,
    ),
...
export const encounterParticipants = pgTable(
  ...
  (table) => [
    ...
    index("encounter_participants_user_idx").on(
      table.userId,
      table.encounterId,
    ),
  ],
...
export const messages = pgTable(
  ...
  (table) => [
    uniqueIndex("messages_thread_client_uidx").on(
      table.threadId,
      table.clientMessageId,
    ),
    uniqueIndex("messages_thread_sequence_uidx").on(
      table.threadId,
...
export const friendRequests = pgTable(
  "friend_requests",
  {
    ...
    recipientId: uuid("recipient_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    sourceEncounterId: uuid("source_encounter_id").references(
      () => encounters.id,
      { onDelete: "set null" },
    ),
