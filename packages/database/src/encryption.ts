...
export function createAesGcmFieldEncryptor(key: Uint8Array, keyVersion: string): FieldEncryptor {
  if (key.byteLength !== 32) throw new Error('Field encryption key must contain exactly 32 bytes')
  ...
  return {
    ...
    decrypt(value) {
      ...
      if (!noncePart || !tagPart || !bodyPart) throw new Error('Malformed encrypted field')
