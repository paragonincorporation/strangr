...
export class EngagementRepository {
  ...
  async markConnected(encounterId: string, now = new Date()) {
    await this.db
      .update(encounters)
      .set({
        connectedAt: now,
  ...
  async finalizeTiming(encounterId: string, now = new Date()) {
  ...
  async ratingSummary(userId: string) {
    const [row] = await this.db
      .select({
        totalLikes: sql<number>`count(*) filter (where ${conversationRatings.outcome} = 'like')::int`,
  ...
  async ratingAnomalySummary(now = new Date()) {
    ...
    const row = result.rows[0] as
      { repeated_pairs?: number; coordinated_like_pairs?: number } | undefined;
    return {
      windowHours: 24,
  ...
  async revokePair(first: string, second: string, now = new Date()) {
    await this.db
  ...
  private async revokeReveal(id: string, now: Date) {
    await this.db
      .update(encounterIdentityReveals)
      .set({ revokedAt: now })
      .where(eq(encounterIdentityReveals.id, id));
  }
  private async blocked(first: string, second: string) {
    const [row] = await this.db
      .select({ id: blocks.id })
      .from(blocks)
      .where(
  ...
  private async assertPairEligible(first: string, second: string) {
