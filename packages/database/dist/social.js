...
export class BlockRepository {
  ...
  async hasEitherDirection(first, second) {
  ...
  async create(blockerId, blockedId, reasonCategory) {
  ...
  async blocksUsername(viewerId, username) {
...
export class EncounterRepository {
  ...
  async start(id, mode, participantIds, now = new Date()) {
  ...
  async connected(id, now = new Date()) {
    await this.database
      .update(calls)
      .set({ connectedAt: now })
      .where(and(eq(calls.encounterId, id), isNull(calls.connectedAt)));
  }
  async end(id, reason, actorId, diagnostic, now = new Date()) {
    await this.database.transaction(async (tx) => {
      await tx
        .update(encounters)
        .set({
  ...
  async list(userId, cursor, limit, now = new Date()) {
  ...
  async preserveEvidence(encounterId, messageId, reason, expiresAt) {
