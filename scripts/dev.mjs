...
for (const port of ports) {
  if (!(await portIsAvailable(port.value)))
    occupied.push(`${port.name} (${port.value})`);
}

if (occupied.length) {
  console.error(
    `Cannot start Paramingle; these ports are already in use: ${occupied.join(", ")}`,
  );
  console.error(
    "Set WEB_PORT, ADMIN_PORT, or API_PORT to unused values and retry.",
  );
  process.exit(1);
}
...
console.info("Paramingle development services:");
...
const child = spawn(
  ...
  [
    ...
    "--prefix-colors",
    "magenta,cyan,green",
    "npm run dev --workspace @paramingle/web",
    "npm run dev --workspace @paramingle/admin",
    "npm run dev --workspace @paramingle/api",
  ],
